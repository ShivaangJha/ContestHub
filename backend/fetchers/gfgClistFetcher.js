...
async function fetchGfgContestsFromClist() {
  try {
    ...
    if (contestsRaw.length === 0) {
      console.log("⚠️ No upcoming GFG contests from clist.by");
      return;
    }

    const mapped = contestsRaw.map((c) => ({
      name: c.event || c.title || "GFG Contest",
      platform: "GFG",
      startTime: toDate(c.start),
      endTime: toDate(c.end) || toDate(c.finish) || toDate(c.start),
      url: c.href || "https://www.geeksforgeeks.org/contest/"
    ...
    try {
      ...
      console.log(`✅ GFG contests (clist.by) saved to MongoDB (${mapped.length})`);
    } catch (insertError) {
    ...
    console.error("❌ Error fetching GFG contests from clist.by:", err);
  }
}

module.exports = fetchGfgContestsFromClist;
